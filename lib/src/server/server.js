...
app.post("/updatePlan/*", (req, res) => {
	...
	if (Object.keys(plans).indexOf(id) !== -1) {
		console.log(checkForDoubleEntry(req.body));
		if (!checkForDoubleEntry(req.body)) {
			plans[id] = req.body;
			fileHandler.write("../db/plans.json", plans, () => {
				res.sendStatus(200);
			});
...
app.post("/updatePlanName/*", (req, res) => {
	console.log("POST PLAN REQUESTED");
	let url = decodeURI(req.url);
	let id = url.substring(url.lastIndexOf("/") + 1);
	id = id.split("%20").join(" ");
	console.log(req.body);
	if (Object.keys(plans).indexOf(id) !== -1) {
		if (Object.keys(plans).indexOf(req.body.name) === -1) {
			plans[req.body.name] = plans[id];
			var i = id;
			fileHandler.write("../db/plans.json", plans, () => {
				let index = i;
				fileHandler.read("../db/plans.json", (data) => {
					plans = data;
					delete plans[index];
					plans[req.body.name].name = req.body.name;
					fileHandler.write("../db/plans.json", plans, () => {
						res.sendStatus(200);
					});
				});
...
app.post("/setModules/*", (req, res) => {
	...
	let id = url.substring(url.lastIndexOf("/") + 1);
	id = id.split("%20").join(" ");
	console.log(Object.keys(modules).indexOf(id));
	if (Object.keys(modules).indexOf(id) === -1) {
		modules[id] = req.body;
		console.log(req.body);
		fileHandler.write("../db/modules.json", modules, () => {
			res.status(409).send();
		});
	}
	else {
...
app.post("/updateModules/*", (req, res) => {
	console.log("UPDATE MODULE");
	let url = decodeURI(req.url);
	let id = url.substring(url.lastIndexOf("/") + 1);
	id = id.split("%20").join(" ");
	if (Object.keys(modules).indexOf(id) !== -1) {
		modules[req.body.name] = req.body;
		if (req.body.name !== id) {
			delete modules[id];
		}
		console.log(req.body);
...
function checkForDoubleEntry(entry) {
	let p = entry;
	let entries = {};
	let b = false;
	p.semesters.forEach(semester => {
		semester.modules.forEach(m => {
			if (!entries.hasOwnProperty(m.name) && m.name !== "wpf") {
				entries[m.name] = m;
			}
			else if (m.name === "wpf") {
				console.log("wpf");
...
fileHandler.read("../db/modules.json", (data) => {
	modules = data;
});
fileHandler.read("../db/plans.json", (data) => {
	plans = data;
