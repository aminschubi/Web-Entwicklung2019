...
class ModulListElement {
	constructor(modul) {
		if (modul !== undefined) {
			...
			this.div.className = "modulListElement";
			this.buttons = [];

			this.nameButton = document.createElement("button");
			this.nameButton.className = "MnameButton";
			...
			this.editButton.className = "MeditB";
			...
			this.deleteButton.className = "MdeleteB";
			...
			this.addButton.onclick = (event) => {
				if (this.add()) {
					...
					let listelement = new ModulListElement(undefined);
			...
			this.flipPageButtonPos.onclick = (event) => {
				if (planPage < modules.length) {
					planPage++;
					reloadM();
				}
			};

			if (modules[planPage - 1].length === 5 && modules[planPage] !== undefined) {
			...
			if (modules[planPage - 2] !== undefined) {
	...
	add() {
		...
		if (name === "") {
			...
			if (!Number.isNaN(ects) && ects >= 30) {
				console.log("name entered");
				let httpReq = new XMLHttpRequest();
				httpReq.open("POST", "http://localhost:8080/setModule/" + name);
...
function readModules(cb) {
	...
	httpReq.onreadystatechange = (event) => {
		if (httpReq.readyState === 4 && httpReq.status === 200) {
			...
			modules = JSON.parse(httpReq.responseText);
			if (Object.keys(modules).length > 0) {
				let i = 0;
				let k = 0;
				for (var key in parsedJSON) {
					if (k < 5) {
						mList[i].push(modules[key]);
						k++;
					}
					else {
						i++;
