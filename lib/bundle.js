...
class SemesterListElement {
	constructor(semester, index, parent) {
		...
		for (var i = 0; i < x; i++) {
			...
			//console.log("100/(" + (((this.plan.plan.ectsPerSemester / this.plan.client.globalSetMinimum) + 1) * (semester.modules[i].ects / this.plan.client.globalSetMinimum)) + ")%");
			...
			b.onclick = (event) => {
				...
				if (freeEctsPoints >= this.plan.client.selectedModule.ects) {
					...
					this.plan.client.updatePlan(this.plan.plan, (value) => {
						if (value === 1) {
							/*for (let i = 0; i < (this.plan.plan.semesters[index - 1].modules[(Number.parseInt(event.target.name))].ects / this.plan.client.globalSetMinimum) - 1; i++) {
								this.plan.plan.semesters[index - 1].modules.forEach((element, ind) => {
									if (element.name === "wpf") {
										this.plan.plan.semesters[index - 1].modules.splice(ind, 1);
										return;
									}
								});
							}*/
