...
class Client {
	...
	reloadM(t) {
		...
		if (t.modules[t.planPage - 1] !== undefined && t.modules[t.planPage - 1].length === 5 && t.modules[t.planPage] !== undefined) {
			t.flipPageButtonPos.disabled = false;
		}
		else {
			t.flipPageButtonPos.disabled = true;
		...
		if (Object.keys(t.modulesObject).length > 0) {
			...
			let k = t.modules[t.planPage - 1].length < 5 ? t.modules[t.planPage - 1].length : 5;
...
class PlanListElement {
	...
	delete() {
		...
		httpReq.onreadystatechange = (event) => {
			if (httpReq.status === 200) {
				console.log("RESPONSE TO POST ARRIVED");
				this.client.readPlans(this.client.reloadList(this.client));
			}
		};
		httpReq.send();
	}
